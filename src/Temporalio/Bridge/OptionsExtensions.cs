...
#pragma warning restore 0618
                if (!options.DeploymentOptions.UseWorkerVersioning
                    && options.DeploymentOptions.DefaultVersioningBehavior != Common.VersioningBehavior.Unspecified)
                {
                    throw new ArgumentException(
                        "DefaultVersioningBehavior must be Unspecified when UseWorkerVersioning is false");
                }
...
#pragma warning restore 0618
            ...
            var pluginNames = workerPluginNames
                ...
                .Distinct()
                .ToArray();
            var storageDriverNames = options.StorageDrivers?
                .Select(d => d.Name)
                .Distinct()
            ...
            return new()
            {
                ...
                storage_drivers = scope.ByteArrayArray(storageDriverNames),
            };
        }
            ...
            this Temporalio.Worker.WorkflowReplayerOptions options, Scope scope)
        {
            ...
            return new()
            {
                ...
                nondeterminism_as_workflow_fail_for_types = scope.ByteArrayArray(
                    AllNonDeterminismFailureTypeWorkflows(options.Workflows)),
                plugins = scope.ByteArrayArray(pluginNames),
                storage_drivers = scope.ByteArrayArray(Array.Empty<string>()),
            };
        }
